...
class MyConv2dSVD(torch.nn.modules.conv.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1,
                 bias=True, padding_mode='zeros', is_train_or_inference=True):
    ...
    def reshape_weight(self):
        ...
        w_r = multi_dot([u, s_diag, v_t])

        w = w_r.t().view(self.out_channels, self.in_channels, self.kernel_size[0], self.kernel_size[1])

        self.weight.data = torch.nn.Parameter(w)
...
#
#         convolvedOutput = multi_dot([i_t.squeeze(), self.weight.view(self.weight.size(0), -1).t()]).unsqueeze(
#             0).transpose(
#             1, 2)
#
...
#         v_t = v.t()
#
#         convolvedOutput = multi_dot([inputUnfolded.transpose(1, 2).squeeze(), u, s_diag, v_t]).unsqueeze(0).transpose(1,
#                                                                                                                       2)
#
#         if self.bias is not None:
#             convolvedOutput += self.bias.view(-1, 1)
